...
// El concepto ya no desaparece en ingreso e inversión: se queda en su hueco bloqueado en
// gris para que la rejilla mida lo mismo en los cuatro tipos. Lo que no puede cambiar es
// la garantía de fondo: bloqueado no es obligatorio, no arrastra valor y no se envía.
function registrarDom(app, type) {
  ...
  const conceptInput = { value: "Vivienda", required: true, disabled: false };
  ...
  return { classes, conceptField, typeInput, conceptInput };
}

test("el concepto se bloquea, deja de ser obligatorio y se limpia para ingreso e inversión", () => {
  for (const type of ["Ingreso", "Inversión"]) {
    const app = loadApp();
    const dom = registrarDom(app, type);
    app.syncRegistrarConceptField();

    assert.equal(dom.conceptInput.disabled, true, type);
    assert.equal(dom.conceptInput.required, false, type);
    assert.equal(dom.conceptInput.value, "", type);
  }
});

test("la resincronización del modo periódico no puede volver a soltar el concepto", () => {
  const app = loadApp();
  const dom = registrarDom(app, "Inversión");
  dom.conceptInput.disabled = false; // Simula que el ajuste de campos periódicos lo ha soltado.
  app.syncRegistrarConceptField();

  assert.equal(dom.conceptInput.disabled, true);
});

test("el concepto se conserva, es obligatorio y se puede tocar para gasto", () => {
  const app = loadApp();
  const dom = registrarDom(app, "Gasto");
  app.syncRegistrarConceptField();

  assert.equal(dom.conceptInput.disabled, false);
...
// En transferencia no hay descripción que valga, pero el campo se queda a la vista
// bloqueado para no cambiar la forma de la rejilla. El que desaparece es el desplegable de
// carteras, que ahí no tiene ningún sentido.
test("en transferencia la descripción se queda bloqueada y sin desplegable de carteras", () => {
  const app = loadApp();
  const dom = descriptionDom(app, "Transferencia");
  dom.shown.set("free", false);
  dom.shown.set("investment", true);
  ...
  assert.equal(dom.shown.get("free"), true);
  assert.equal(dom.shown.get("investment"), false);
  assert.equal(dom.descriptionInput.disabled, true);
  assert.equal(dom.descriptionInput.required, false);
